...
string Solution::longestCommonPrefix(vector<string> &A) {
    ...
    int i=0;
    while(i<n && A[0][i]==A[m][i]){   //first and last string will be most dissimilar thus finding common charcters in these
        s+=A[0][i];
        i++;
    }
    return s;
...
string commonPrefix(string arr[], int n){
    ...
    for (int i=0; i<minlen; i++){
        current = arr[0][i];        // Current character (must be same in all strings to be a part of result)
