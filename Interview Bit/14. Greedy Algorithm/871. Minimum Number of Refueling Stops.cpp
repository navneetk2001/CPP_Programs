...
//Solution Approach :- O(nlog(n)) using max-heap
...
public:
    int minRefuelStops(int target, int startFuel, vector<vector<int>>& stations) {
        ...
        /*	if we can't reach target, I will first use the largestfuel Station(which can take me farthest) and move ahead and keep doing this.    */
        ...
        while(curr < target) {          //until I reach my target
            while(i < n && curr >= stations[i][0]) {
                /*  Sice, I am already ahead of or at this station (i.e. curr >= stations[i][0])  we don't use this station and try to move ahead but I store this station in maxheap to use in future */
...
public:
    int minRefuelStops(int target, int startFuel, vector<vector<int>>& stations) {
        ...
        //t[i][j] = Maximum distance reached if I stopped at 'j' fuelStops from total of 'i' fuelStops i.e. j <= i
        ...
        //There is no point of filling 1st row because t[0][j] means I used j fuelStops from 0 fuelStops which doesn't make any sense for (j>0)
        ...
        for(int i = 1; i < n+1; i++) {
            for(int j = 1; j <= i; j++) {           //I can use j=1 to j=i fuel stops and find max distance for t[i][j]
                //case-1 (I don't refuel at current fuelstop) If I didn't refuel at current stop, the max distance I can cover is equal to maximum distance I could cover from previous state
                t[i][j] = t[i-1][j];
                
                //case-2 (I will refuel at current fuelStop). But check if you can even reach this station (stations[i-1][0]) or not from your previous station
        ...
        //After you end up with all stations, check which t[n][j] was best for reaching target Best means, minimum j used by you to reach target
        for(int j = 0; j<n+1; j++) {
            if(t[n][j] >= target){
                return j;
            }
